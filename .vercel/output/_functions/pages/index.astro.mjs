...
import { c as getReviews } from '../chunks/wpapi_BARwLM4G.mjs';
...
const $$Services = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section class="rounded-tr-[200px] px-8 md:px-14 lg:px-40 xl:px-50 pb-20 pt-10 z-1 relative border-b-2 border-dashed border-gray-400/10"> <h2 class="text-3xl sm:text-7xl text-primary mb-2 text-center">What We Do</h2> <!-- <p class="text-secondary text-sm sm:max-w-3/4 mb-12">Lorem ipsum dolor sit amet consectetur adipisicing elit. Eaque temporibus perspiciatis animi a? Nulla quisquam voluptatum doloribus provident eius dignissimos!</p> --> <div class="text-2xl md:text-4xl mt-20 flex font-extrabold gap-x-4 justify-center items-center"> <span class="text-zinc-100">Creative</span> ${renderComponent($$result, "RotatingText", null, { "client:only": true, "texts": ["websites", "AI solutions", "designs", "marketing", "apps"], "mainClassName": "px-2 md:px-4 background-theme overflow-hidden py-3 justify-center rounded-lg", "staggerFrom": "last", "initial": { y: "100%" }, "animate": { y: 0 }, "exit": { y: "-120%" }, "staggerDuration": 0.025, "splitLevelClassName": "overflow-hidden pb-0.5 sm:pb-1 md:pb-1", "transition": { type: "spring", damping: 30, stiffness: 400 }, "rotationInterval": 2e3, "client:component-hydration": "only", "client:component-path": "C:/Users/jarndev/dev/xlynxai.vercel.app/src/components/RotatingText", "client:component-export": "default" })} </div> <div class="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-8 mt-20"> ${mockData.map((service, index) => {
    if (service.href) {
...
const $$Testimonials = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section id="reviews" class="px-8 bg-white rounded-r-full lg:px-40 xl:px-50 md:px-14 py-30 pb-20 relative z-1 border-b-2 border-dashed border-gray-400/10"> <h1 class="text-2xl sm:text-7xl text-center text- mb-10">Success Stories</h1> <div class="flex justify-center sm:*:max-w-6/8"> <div class="swiper"> <div class="swiper-wrapper"> ${reviews?.map((r) => renderTemplate`${renderComponent($$result, "TestimonialCard", $$TestimonialCard, { "author": r.author, "authorPosition": r.authorPosition, "authorCompany": r.authorCompany, "message": r.message })}`)} </div> </div> </div> </section> ${renderScript($$result, "C:/Users/jarndev/dev/xlynxai.vercel.app/src/components/Testimonials.astro?astro&type=script&index=0&lang.ts")}`;
